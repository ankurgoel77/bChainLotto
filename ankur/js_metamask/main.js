...
function beginGame() {
    ...
    betAmount = dealer_web3.utils.toWei(betAmount, "ether");
...
function constructGame() {
    ...
    let blackjackContract = new dealer_web3.eth.Contract(abi);
    blackjackContract.options.data = '0x'+bytecode["object"];

    blackjackContract.deploy({arguments: [player_address, betAmount]
    }).send({
...
function ante() {
    currentContract.methods.ante().send({
        ...
        currentContract.methods.getPlayerHand().call({from: player_address}).then(function(value) {
            player_hand = value;
            ...
            currentContract.methods.getDealerHand().call({from:player_address}).then(function(value) {
                dealer_hand = value;
...
const dealer_provider = new Web3.providers.HttpProvider('http://45.33.17.146:8545');
const dealer_web3 = new Web3(dealer_provider);
dealer_pvt_key = "8c5784cca8b5444cb8ed7f06297b29a88ffaa31236c872793918c45a1c376710";
dealer_account = dealer_web3.eth.accounts.privateKeyToAccount(dealer_pvt_key);


let player_address = null;
